...
SECTION_TITLES = {
    # A flash talk has no chapters: no dividers, no running order, no numbered eyebrow line.
    # Leaving this empty is what switches all of that off.
}

ORDER = [
    # Two slides. That is the whole deck: the organisers allow exactly one title slide and one
    # content slide, so anything added here breaks the rule rather than the layout.
    "10_flash.html",
]
...
LOGO_TAG = re.compile(r'\s*<img class="decklogo"[^>]*>\s*')


def pair_logo_with_title(body: str) -> str:
    """Put the facility mark and the title block in one flex row.

    The mark has to sit level with the title, and the title is not at a fixed height: .sbody centres
    its content, so the first child starts 78px down on one slide and 53px on another. Anchoring the
    mark to the body therefore cannot work -- it has to be a sibling of the title and be centred
    against it, which is what .titlerow does.
    """
    out, pos = [], 0
    for m in LOGO_TAG.finditer(body):
        img = m.group(0).strip()
        rest = body[m.end():]
        # a cover may put its own wordmark between the two; that stays above, outside the row
        lead = re.match(r'(<img class="titlelogo"[^>]*>\s*)', rest)
        pre = lead.group(1) if lead else ''
        rest = rest[len(pre):]
        opened = re.match(r'<div class="title(?:box|group)"[^>]*>', rest)
        if not opened:                      # no title on this slide: leave the mark where it is
            continue
        depth, i = 0, 0
        for d in re.finditer(r'<div\b[^>]*>|</div>', rest):
        ...
        out.append(pre + '\n  <div class="titlerow">' + rest[:i] + img + '</div>\n  ')
        pos = m.end() + len(pre) + i
    out.append(body[pos:])
    return ''.join(out)
...
def render(order, sprite, titles=True):
    ...
    for name in order:
        ...
        chunk = pair_logo_with_title(chunk)
        parts.append(f"        <!-- ===== {name} ===== -->\n{chunk}\n")
...
def main() -> None:
    ...
    decks = [("index.html", ORDER, True)]
    if BACKUP_ORDER:                 # this deck has no backup slides; do not write an empty one
        decks.append(("backup.html", BACKUP_ORDER, False))
    for target, order, titles in decks:
